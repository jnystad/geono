...
function run() {
  ...
  // Parse CSW XML records
  fs.readdirSync("./data").forEach((file) => {
    //if (!file.startsWith("0a")) return;
    ...
    fs.readFile(`./data/${file}`, "utf8", (err, data) => {
      ...
      for (const constraint of resourceConstraints) {
        ...
        if (accessConstraints) {
          ...
          if (constraints.accessConstraints?.includes("noLimitations"))
            constraints.accessConstraints = "no restrictions";
      ...
      for (const graphic of graphicOverview) {
        ...
        const fileDescription = graphic
          .querySelector("gmd\\:filedescription gco\\:CharacterString")
          ?.textContent.trim();
      ...
      fs.writeFileSync(`./records/${id}.json`, JSON.stringify(output, null, 2));
    });
  });
}
